...
sc_pkcs15_bind_internal(struct sc_pkcs15_card *p15card, struct sc_aid *aid)
{
	...
	if (info)   {
		...
		/* Fallback: use first app when aid lookup fails (e.g. PIV/CCID encoding) */
		if (card->app_count > 0 && card->app[0]) {
			info = card->app[0];
			sc_log(ctx, "Application '%s' not found, using first app '%s'", sc_dump_hex(aid->value, aid->len), info->label);
			sc_pkcs15_free_app(p15card);
			p15card->app = sc_dup_app_info(info);
			if (p15card->app) {
				if (info->path.len)
