...
pkcs15_bind(struct sc_pkcs11_card *p11card, struct sc_app_info *app_info)
{
	...
	if (rc != SC_SUCCESS) {
		...
		OPENSC_LOGD("pkcs15_bind failed rc=%d (%s)", rc, sc_strerror(rc));
...
CK_RV C_GetTokenInfo(CK_SLOT_ID slotID, CK_TOKEN_INFO_PTR pInfo)
{
	...
	if (auth) {
		...
		pin_rc = sc_pkcs15_get_pin_info(p15card, auth);
		if (pin_rc != SC_SUCCESS) {
			/* e.g. 63 CX not converted to success for GET_INFO - use safe defaults */
			sc_log(context, "C_GetTokenInfo: get_pin_info failed (%d), using defaults", pin_rc);
...
int slot_get_logged_in_state(struct sc_pkcs11_slot *slot)
{
	...
	if (!pin_info)
		goto out;
	if (sc_pkcs15_get_pin_info(p15card, pin_obj) != SC_SUCCESS) {
		/* e.g. 63 CX not converted - assume logged out, do not propagate */
...
pkcs15_create_tokens(struct sc_pkcs11_card *p11card, struct sc_app_info *app_info)
{
	...
	if (rc < 0) {
		OPENSC_LOGD("create_tokens: _pkcs15_create_typed_objects failed rc=%d", rc);
		return sc_to_cryptoki_error(rc, NULL);
	}
	...
	if (!auth_user_pin || cs_flags & SC_PKCS11_SLOT_CREATE_ALL)   {
		...
		if (rc < 0) {
			OPENSC_LOGD("create_tokens: sc_pkcs15_get_objects AUTH_PIN failed rc=%d", rc);
			return sc_to_cryptoki_error(rc, NULL);
		}
		auth_count = rc;
