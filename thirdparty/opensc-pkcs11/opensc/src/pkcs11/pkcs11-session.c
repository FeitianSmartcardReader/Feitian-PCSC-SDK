...
/* opensc-pkcs11 for Android build only; logcat enabled unconditionally */
#include <android/log.h>
#define OPENSC_LOGD(fmt, ...) __android_log_print(ANDROID_LOG_INFO, "PKCS11Demo", fmt, ##__VA_ARGS__)
...
{				/* receives new session handle */
	...
	OPENSC_LOGD("C_OpenSession [0] entry slotID=0x%lX flags=0x%lX phSession=%p", (unsigned long)slotID, (unsigned long)flags, (void *)phSession);
	sc_log(context, "C_OpenSession(slotID=0x%lX, flags=0x%lX, phSession=%p)",
	       (unsigned long)slotID, (unsigned long)flags, (void *)phSession);

	if (phSession == NULL_PTR) {
		OPENSC_LOGD("C_OpenSession [1] phSession NULL -> return CKR_ARGUMENTS_BAD");
		sc_log(context, "C_OpenSession: phSession NULL -> CKR_ARGUMENTS_BAD");
		return CKR_ARGUMENTS_BAD;
	}

	if (!(flags & CKF_SERIAL_SESSION)) {
		OPENSC_LOGD("C_OpenSession [2] missing CKF_SERIAL_SESSION -> return CKR_SESSION_PARALLEL_NOT_SUPPORTED");
		sc_log(context, "C_OpenSession: missing CKF_SERIAL_SESSION -> CKR_SESSION_PARALLEL_NOT_SUPPORTED");
		return CKR_SESSION_PARALLEL_NOT_SUPPORTED;
	}

	/* Ignore unknown flags instead of rejecting (e.g. CKF_DONT_BLOCK) */
	flags &= (CKF_SERIAL_SESSION | CKF_RW_SESSION);

	OPENSC_LOGD("C_OpenSession [3] calling sc_pkcs11_lock");
	rv = sc_pkcs11_lock();
	if (rv != CKR_OK) {
		OPENSC_LOGD("C_OpenSession [4] lock failed rv=0x%08lX -> return", (unsigned long)rv);
		sc_log(context, "C_OpenSession: lock failed rv=0x%08lX", (unsigned long)rv);
		return rv;
	}

	OPENSC_LOGD("C_OpenSession [5] calling slot_get_token slotID=0x%lX", (unsigned long)slotID);
	rv = slot_get_token(slotID, &slot);
	if (rv != CKR_OK) {
		OPENSC_LOGD("C_OpenSession [6] slot_get_token failed rv=0x%08lX -> goto out", (unsigned long)rv);
		sc_log(context, "C_OpenSession: slot_get_token failed rv=0x%08lX (slotID=0x%lX)",
		       (unsigned long)rv, (unsigned long)slotID);
		goto out;
	}

	OPENSC_LOGD("C_OpenSession [7] slot_get_token OK, check RW_SESSION vs login_user");
	/* Check that no conflicting sessions exist */
	if (!(flags & CKF_RW_SESSION) && (slot->login_user == CKU_SO)) {
		OPENSC_LOGD("C_OpenSession [8] CKR_SESSION_READ_WRITE_SO_EXISTS -> goto out");
		rv = CKR_SESSION_READ_WRITE_SO_EXISTS;
		goto out;
	}

	OPENSC_LOGD("C_OpenSession [9] calloc session");
	session = (struct sc_pkcs11_session *)calloc(1, sizeof(struct sc_pkcs11_session));
	if (session == NULL) {
		OPENSC_LOGD("C_OpenSession [10] calloc failed CKR_HOST_MEMORY -> goto out");
		rv = CKR_HOST_MEMORY;
		goto out;
	}
	...
	OPENSC_LOGD("C_OpenSession [11] check handle uniqueness");
	if (list_seek(&sessions, &session->handle) != NULL) {
		OPENSC_LOGD("C_OpenSession [12] handle already exists CKR_HOST_MEMORY -> goto out");
	...
	OPENSC_LOGD("C_OpenSession [13] append session, success");
	session->slot = slot;
...
out:
	OPENSC_LOGD("C_OpenSession [out] rv=0x%08lX", (unsigned long)rv);
