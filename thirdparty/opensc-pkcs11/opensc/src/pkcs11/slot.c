...
again:
	...
	if (rc < 0) {
		...
		sc_log(context, "%s: sc_detect_card_presence failed rc=%d (%s) -> CKR 0x%08lX",
		       reader->name, rc, sc_strerror(rc), (unsigned long)rv);
		OPENSC_LOGD("%s: sc_detect_card_presence failed rc=%d (%s) -> CKR 0x%08lX",
		       reader->name, rc, sc_strerror(rc), (unsigned long)rv);
		return rv;
	}
	if (rc == 0) {
	...
	if (p11card->card == NULL) {
		...
		if (rc != SC_SUCCESS) {
			rv = sc_to_cryptoki_error(rc, NULL);
			sc_log(context, "%s: sc_connect_card failed rc=%d (%s) -> CKR 0x%08lX",
			       reader->name, rc, sc_strerror(rc), (unsigned long)rv);
			OPENSC_LOGD("%s: sc_connect_card failed rc=%d (%s) -> CKR 0x%08lX",
			       reader->name, rc, sc_strerror(rc), (unsigned long)rv);
			goto fail;
		}
	...
	if (p11card->framework == NULL) {
		...
		if (app_generic || !p11card->card->app_count)   {
			...
			sc_log(context, "%s: bind 'generic' token returned 0x%lX", reader->name, rv);
			OPENSC_LOGD("%s: bind 'generic' token returned 0x%lX", reader->name, (unsigned long)rv);
			...
			sc_log(context, "%s: create_tokens 'generic' returned 0x%lX", reader->name, rv);
			OPENSC_LOGD("%s: create_tokens 'generic' returned 0x%lX", reader->name, (unsigned long)rv);
			if (rv != CKR_OK)   {
				sc_log(context,
		...
		for (j = 0; j < p11card->card->app_count; j++)   {
			...
			sc_log(context, "%s: bind %s returned 0x%lX", reader->name, app_name, rv);
			if (rv != CKR_OK)   {
			...
			sc_log(context, "%s: create_tokens %s returned 0x%lX", reader->name, app_name, rv);
			if (rv != CKR_OK)   {
				sc_log(context,
...
CK_RV slot_get_token(CK_SLOT_ID id, struct sc_pkcs11_slot ** slot)
{
	...
	if (rv != CKR_OK) {
		OPENSC_LOGD("slot_get_token: slot_get_slot failed rv=0x%08lX (slotID=0x%lX)", (unsigned long)rv, (unsigned long)id);
		return rv;
	}
	...
	if (!((*slot)->slot_info.flags & CKF_TOKEN_PRESENT)) {
		...
		if (rv != CKR_OK) {
			sc_log(context, "slot_get_token: card_detect failed rv=0x%08lX", (unsigned long)rv);
			OPENSC_LOGD("slot_get_token: card_detect failed rv=0x%08lX (slotID=0x%lX)", (unsigned long)rv, (unsigned long)id);
			return rv;
		}
	}
