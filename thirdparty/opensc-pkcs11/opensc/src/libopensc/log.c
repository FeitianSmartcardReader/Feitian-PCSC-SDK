...
static void sc_do_log_va(sc_context_t *ctx, int level, const char *file, int line, const char *func, int color, const char *format, va_list args)
{
...
#if defined(__ANDROID__) || defined(ANDROID)
	{
		...
		va_list args_copy;
		if (file != NULL)
			off = (size_t)snprintf(buf, sizeof(buf), "%s:%d:%s: ", file, line, func ? func : "");
		va_copy(args_copy, args);
		off += (size_t)vsnprintf(buf + off, sizeof(buf) - off, format, args_copy);
		va_end(args_copy);
		if (off > 0 && buf[off - 1] != '\n' && off < sizeof(buf) - 1)
			strlcat(buf, "\n", sizeof(buf));
		__android_log_print(ANDROID_LOG_DEBUG, "OpenSC", "%s", buf);
		return;
	}
#endif
