...
#define PIV_LOG(fmt, ...) __android_log_print(ANDROID_LOG_DEBUG, "PIV", fmt, ##__VA_ARGS__)
#else
#define PIV_LOG(fmt, ...) ((void)0)
#endif
	...
	size_t recvbuflen)
{
	...
	PIV_LOG("piv_general_io: ins=0x%02X p1=0x%02X p2=0x%02X", ins, p1, p2);
	SC_FUNC_CALLED(card->ctx, SC_LOG_DEBUG_VERBOSE);
...
static int piv_sm_open(struct sc_card *card)
{
	...
	PIV_LOG("piv_sm_open: secure messaging key establishment");
	int i;
	int reps;
	u8 CBh;
	u8 CBicc;
...
static int piv_find_aid(sc_card_t * card)
{
	...
	PIV_LOG("piv_find_aid: select PIV AID");
	SC_FUNC_CALLED(card->ctx, SC_LOG_DEBUG_VERBOSE);
	...
	unsigned int key_ref, unsigned int alg_id)
{
	...
	PIV_LOG("piv_general_mutual_authenticate: key_ref=%u alg_id=0x%02X", key_ref, alg_id);
	SC_FUNC_CALLED(card->ctx, SC_LOG_DEBUG_VERBOSE);
		...
		unsigned int key_ref, unsigned int alg_id)
{
	...
	PIV_LOG("piv_general_external_authenticate: key_ref=%u alg_id=0x%02X", key_ref, alg_id);
	SC_FUNC_CALLED(card->ctx, SC_LOG_DEBUG_VERBOSE);
...
piv_pin_cmd(sc_card_t *card, struct sc_pin_cmd_data *data, int *tries_left)
{
	...
	sc_log(card->ctx, "piv_pin_cmd: iso pin_cmd returned %d, pin_cmd_verify_sw1=0x%02X sw2=0x%02X",
		r, priv->pin_cmd_verify_sw1, priv->pin_cmd_verify_sw2);
	...
	if (data->cmd == SC_PIN_CMD_GET_INFO) {
		...
		/* NIST SP 800-73-4: 63 CX is valid for VERIFY Lc=0 (GET_INFO) - always treat as success */
		if (priv->pin_cmd_verify_sw1 == 0x63U && r != SC_SUCCESS) {
			sc_log(card->ctx, "GET_INFO 63 CX: forcing success, tries_left=%d", priv->pin_cmd_verify_sw2 & 0x0F);
			data->pin1.tries_left = priv->pin_cmd_verify_sw2 & 0x0F;
			data->pin1.logged_in = SC_PIN_STATE_LOGGED_OUT;
			r = SC_SUCCESS;
		}
